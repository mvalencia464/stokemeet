          ...
          items.map((item) => (
            <div key={item.id} className="flex gap-3 group">
              <input 
                ...
                className="mt-1 w-4 h-4 rounded border-[#30363d] bg-[#161b22] text-[#0070f3] focus:ring-0 cursor-pointer"
              />
              <div className="flex flex-col">
                <p className={`text-sm leading-relaxed ${item.completed ? 'line-through text-[#8b949e]' : 'text-[#e6edf3]'}`}>
                  {item.text}
