      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="text-[10px] bg-[#ccff00]/10 px-2 py-1 border border-[#ccff00]/30 rounded text-[#ccff00] flex items-center gap-1 disabled:opacity-50"
          >
            {isGeneratingEmail ? (
              <>
          ...
          items.map((item) => (
            <div key={item.id} className="flex gap-3 group items-start cursor-pointer" onClick={() => onToggle(item.id)}>
              <div className={`mt-1 min-w-[1.25rem] h-5 w-5 rounded-md border flex items-center justify-center transition-all ${item.completed
                ...
                : 'bg-[#161b22] border-[#30363d] group-hover:border-[#ccff00]/50'
                }`}>
                {item.completed && <svg className="w-3.5 h-3.5 text-black" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>}
              </div>
              <div className="flex flex-col">
                <p className={`text-sm leading-relaxed transition-colors ${item.completed ? 'line-through text-[#8b949e]' : 'text-[#e6edf3]'}`}>
                  {item.text}
                  {item.timestamp && <span className="ml-2 text-xs text-[#ccff00] opacity-60">✨ @ {item.timestamp}</span>}
                </p>
